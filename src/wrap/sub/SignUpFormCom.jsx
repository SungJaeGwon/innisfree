...
export default function SignUpFormCom () {
    ...
    const onClickDuplicationId=(e)=>{
        ...
        if( value.length > 12 || regexp1.test(value)===false || regexp2.test(value)===true ){
            ...
            axios({
                url: 'http://gksmf519.dothome.co.kr/innisfree/innisfree_id_duplication.php',
                method: 'POST',
                data: formData
            })
            .then(( res )=>{
    ...
    const onClickDuplicationEmail=(e)=>{
        ...
        if( value==='' ){
            ...
            axios({
                url: 'http://gksmf519.dothome.co.kr/innisfree/innisfree_email_duplication.php',
                method: 'POST',
                data: formData
            })
            .then(( res )=>{
    ...
    const onChangeCheckBox=(e)=>{
        ...
        if( e.target.checked===true ){
            if ( e.target.value==='[선택] 개인정보 수집 및 이용 동의 (마케팅)' ){
                if( allAgree.includes('[선택] 뷰티포인트 문자 수신 동의')===false && allAgree.includes('[선택] 온라인 사이트 문자 수신 동의')===false ){
                    allAgree = [...allAgree, e.target.value, '[선택] 뷰티포인트 문자 수신 동의', '[선택] 온라인 사이트 문자 수신 동의' ];
                }
                else if ( allAgree.includes('[선택] 뷰티포인트 문자 수신 동의')===true && allAgree.includes('[선택] 온라인 사이트 문자 수신 동의')===false ){
                    allAgree = [...allAgree, e.target.value, '[선택] 온라인 사이트 문자 수신 동의' ];
                }
                else if ( allAgree.includes('[선택] 뷰티포인트 문자 수신 동의')===false && allAgree.includes('[선택] 온라인 사이트 문자 수신 동의')===true ){
            ...
            if( e.target.value==='[선택] 개인정보 수집 및 이용동의 (마케팅)(이니스프리)' ){
                arr = arr.filter((item)=> item !== e.target.value);
                arr = arr.filter((item)=> item !== '[선택] 이니스프리  문자 수신 동의');
                allAgree = arr;
            }
            else if( e.target.value==='[선택] 이니스프리  문자 수신 동의' ){
                arr = arr.filter((item)=> item !== e.target.value);
                arr = arr.filter((item)=> item !== '[선택] 개인정보 수집 및 이용동의 (마케팅)(이니스프리)');
                allAgree = arr;
            }
            else {
                allAgree = allAgree.filter((item)=> item !== e.target.value)
    ...
    console.log(location.state.birthYear);
    ...
    const onsubmitForm=(e)=>{
        ...
        if( state.id===''){
            ...
            formData.append('userHp', location.state.hp.replace(regExp, '$1-$2-$3'));
            formData.append('userBirth', `${location.state.birthYear}.${location.state.birthMonth}.${location.state.birthDate}` );
            formData.append('userId', state.id);
            formData.append('userPw', state.pw);
            formData.append('userEmail', state.email);
            formData.append('userAddress', `${state.isAddress} ${state.isRemainingAddress}`);
            formData.append('userService', state.allAgree);

            axios({
                url:'http://gksmf519.dothome.co.kr/innisfree/innisfree_insert.php',
                method: 'POST',
                data: formData
            })
            .then(( res )=>{
                if( res.status===200 ){
                    ...
                    if(res.data===1){
                        messageModalMethod('이니스프리 회원가입을 축하드립니다.');
                        navigate('/signIn');
                    }
                    else if(res.data===0) {
                                ...
                                <div className="all_agree_box">
                                    <label htmlFor="beautyAgreeAll">
                                        <input 
                                            ...
                                            checked = {state.allAgree.length === 12 || state.agree.includes('모든 약관 및 정보 수신 동의')}
